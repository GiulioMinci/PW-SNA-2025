...
design_1_folder = "C:\\Users\\giuli\\Desktop\\Scraping comuni\\Esempi\\design_1"
design_2_folder = "C:\\Users\\giuli\\Desktop\\Scraping comuni\\Esempi\\design_2"
...
# Carica i template da entrambe le cartelle
design_1_templates = [os.path.join(design_1_folder, f) for f in os.listdir(design_1_folder) if f.endswith((".png", ".jpg"))]
design_2_templates = [os.path.join(design_2_folder, f) for f in os.listdir(design_2_folder) if f.endswith((".png", ".jpg"))]

# Funzione per filtra i match geometrico
...
# Funzione per normalizzare la dimensione del template
def normalizza_dim(template, image):
    h, w = image.shape[:2]
    return cv2.resize(template, (w, h), interpolation=cv2.INTER_AREA)

# Aggiunta dell'overlay (bollino verde/arancione/rosso)
def aggiungi_overlay(img, conforme, num_matches, design):
    overlay = img.copy()
    colore = (0, 255, 0) if conforme == "Sì" else (0, 0, 255)  # Rosso se non conforme

    if design == "design_2" and num_matches <= design_2_threshold:
        colore = (0, 165, 255)  # Arancione per design_2 con bassa corrispondenza

    bollino_radius = 30
    posizione_bollino = (50, 50)
    posizione_testo = (95, 60)  # Posizione per evitare sovrapposizione
...
# Funzione per verificare la conformità del design
def check_design_conformity(image_path):
    ...
    best_design = ""  # Per tenere traccia del design migliore (design_1 o design_2)

    # Confronto con i template di design_1
    for template_path in design_1_templates:
        ...
        if num_matches > best_matches:
            ...
            best_design = "design_1"

    # Confronto con i template di design_2
    for template_path in design_2_templates:
        template = cv2.imread(template_path, cv2.IMREAD_GRAYSCALE)
        if template is None:
            continue
        template = normalizza_dim(template, image)
        ...
        if num_matches > best_matches:
            ...
            best_kp2 = kp2
            best_match_list = matches_filtrati
            best_design = "design_2"

    # Determina la conformità
    conforme = "Sì" if best_matches > design_1_threshold and best_matches > design_2_threshold else "No"

    if best_template is not None and best_kp1 is not None and best_kp2 is not None and best_match_list is not None:
        # Immagine in bianco e nero con i punti di contatto
        match_img = cv2.drawMatches(best_template, best_kp1, image, best_kp2, best_match_list[:50], None,
                                    flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)

        # Aggiungi l'overlay con bollino e numero di match
        match_img = aggiungi_overlay(match_img, conforme, best_matches, best_design)
        cv2.imwrite(match_img_path, match_img, [int(cv2.IMWRITE_JPEG_QUALITY), 95])

    logging.info(f"{image_path}: {best_matches} matches trovati con {best_design}. Conformità: {conforme}")
    return conforme, best_matches, match_img_path

# Funzione per processare una riga
def process_row(row):
...
# Threshold per design_1 e design_2
design_1_threshold = 35
design_2_threshold = 25
...
# Scrittura dei risultati nel CSV di output
